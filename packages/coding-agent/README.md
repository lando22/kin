...
**Custom providers & models:** Add providers via `~/.kin/agent/models.json` if they speak a supported API (OpenAI, Anthropic, Google). For custom APIs or OAuth, use extensions. See [docs/models.md](docs/models.md) and [docs/custom-provider.md](docs/custom-provider.md).
...
See `/hotkeys` for the full list. Customize via `~/.kin/agent/keybindings.json`. See [docs/keybindings.md](docs/keybindings.md).
...
| `.kin/settings.json` | Project (overrides global) |
...
Replace the default system prompt with `.kin/SYSTEM.md` (project) or `~/.kin/agent/SYSTEM.md` (global). Append without replacing via `APPEND_SYSTEM.md`.
...
Place personal skills in `~/.kin/SKILLS/<skill-name>/`. See [docs/skills.md](docs/skills.md).
...
Read the [blog post](https://mariozechner.at/posts/2025-11-30-kin-coding-agent/) for the full rationale.
...
|----------|-------------|
| `KIN_CODING_AGENT_DIR` | Override config directory (default: `~/.kin/agent`) |
