...
const VIRTUAL_MODULES: Record<string, unknown> = {
	...
	"@mariozechner/kin-ai/oauth": _bundledPiAiOauth,
	"@mariozechner/kin-coding-agent": _bundledPiCodingAgent,
};
...
function getAliases(): Record<string, string> {
	...
	_aliases = {
		...
		"@mariozechner/kin-ai": piAiEntry,
		"@mariozechner/kin-ai/oauth": piAiOauthEntry,
...
function readKinManifest(packageJsonPath: string): KinManifest | null {
	try {
		const content = fs.readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content);
		if (pkg.kin && typeof pkg.kin === "object") {
			return pkg.kin as KinManifest;
		}
		return null;
	} catch {
		return null;
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (fs.existsSync(packageJsonPath)) {
		const manifest = readKinManifest(packageJsonPath);
