import { Markdown, type MarkdownTheme } from "@landongarrison/kin-tui";
...
import { getLatestKinRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|kin] [--self] [--extensions] [--extension <source>] [--force]`;
		case "list":
...
Options:
  -l, --local    Install project-locally (.kin/settings.json)
...
Options:
  -l, --local    Remove from project settings (.kin/settings.json)
...
Update kin and installed packages.

Options:
  --self                  Update kin only
  --extensions            Update installed packages only
  --extension <source>    Update one package only
  --force                 Reinstall kin even if the current version is latest

Short forms:
  ${APP_NAME} update                Update kin and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update kin            Update kin only (self works as alias to kin)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "kin" || source === "pi";
			if (sourceIsSelf) {
