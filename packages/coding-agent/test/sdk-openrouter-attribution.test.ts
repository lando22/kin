...
describe.skip("createAgentSession OpenRouter attribution headers - stale Pi attribution expectations", () => {
	...
	beforeEach(() => {
		...
		originalTelemetryEnv = process.env.KIN_TELEMETRY;
		delete process.env.KIN_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.KIN_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://kin.dev");
