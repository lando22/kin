...
// ============================================================================
// OAuth API key resolution from ~/.kin/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".kin", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.kin/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.kin/agent/auth.json
 */
...
export const PI_AGENT_DIR = join(homedir(), ".kin", "agent");

/**
 * Get an AuthStorage instance backed by ~/.kin/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `kin-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
