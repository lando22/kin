...
describe.skip("DefaultPackageManager - stale package/resource discovery expectations", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.KIN_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.KIN_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .kin skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .kin", async () => {
			const extDir = join(tempDir, ".kin", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".kin"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".kin", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".kin", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".kin", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".kin", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".kin", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .kin/agent skills", async () => {
		...
		it("should use the project .kin dir as baseDir for project .kin skills", async () => {
			const projectBaseDir = join(tempDir, ".kin");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".kin", "agent");
		...
		it("should dedupe user skill entries when ~/.kin/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .kin auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".kin\n");

			const skillDir = join(tempDir, ".kin", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.kin\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".kin", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .kin settings base", () => {
			...
			const rel = relative(join(tempDir, ".kin"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".kin", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".kin", "npm")],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".kin", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".kin", "npm", "node_modules", "project-current");
			...
			expect(runCommandSpy).toHaveBeenNthCalledWith(
				2,
				"npm",
				["install", "project-old@latest", "project-missing@latest", "--prefix", join(tempDir, ".kin", "npm")],
				undefined,
			);
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".kin", "npm", "node_modules", "example");
