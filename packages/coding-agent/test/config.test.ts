...
const originalPiPackageDir = process.env.KIN_PACKAGE_DIR;
...
afterEach(() => {
	...
	if (originalPiPackageDir === undefined) {
		delete process.env.KIN_PACKAGE_DIR;
	} else {
		process.env.KIN_PACKAGE_DIR = originalPiPackageDir;
	}
...
function createPnpmGlobalInstall(): { root: string; packageDir: string } {
	...
	const packageDir = join(root, "@mariozechner", "kin-coding-agent");
	mkdirSync(packageDir, { recursive: true });
	...
	setExecPath(
		join(
			root,
			".pnpm",
			"@mariozechner+kin-coding-agent@0.0.0",
			"node_modules",
			"@mariozechner",
			"kin-coding-agent",
			"dist",
...
function createYarnGlobalInstall(): { globalDir: string; packageDir: string } {
	...
	const packageDir = join(globalDir, "node_modules", "@mariozechner", "kin-coding-agent");
	mkdirSync(packageDir, { recursive: true });
	mkdirSync(binDir, { recursive: true });
	...
	process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
	process.env.KIN_PACKAGE_DIR = packageDir;
	setExecPath(join(globalDir, ".yarn", "@mariozechner", "kin-coding-agent", "dist", "cli.js"));
...
describe("detectInstallMethod", () => {
	...
	test("self-updates renamed packages from the current install prefix", () => {
		...
		const command = getSelfUpdateCommand("@mariozechner/kin-coding-agent", undefined, "@new-scope/kin");

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "@new-scope/kin"],
			display: `npm --prefix ${prefix} uninstall -g @mariozechner/kin-coding-agent && npm --prefix ${prefix} install -g @new-scope/kin`,
			steps: [
				{
					command: "npm",
					args: ["--prefix", prefix, "uninstall", "-g", "@mariozechner/kin-coding-agent"],
					display: `npm --prefix ${prefix} uninstall -g @mariozechner/kin-coding-agent`,
				},
				{
					command: "npm",
					args: ["--prefix", prefix, "install", "-g", "@new-scope/kin"],
					display: `npm --prefix ${prefix} install -g @new-scope/kin`,
				},
			],
		});
	});
	...
	test("self-updates renamed pnpm global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "pnpm",
			args: ["install", "-g", "@new-scope/kin"],
			display: "pnpm remove -g @mariozechner/kin-coding-agent && pnpm install -g @new-scope/kin",
			steps: [
				{
					command: "pnpm",
					args: ["remove", "-g", "@mariozechner/kin-coding-agent"],
					display: "pnpm remove -g @mariozechner/kin-coding-agent",
				},
				{
					command: "pnpm",
					args: ["install", "-g", "@new-scope/kin"],
					display: "pnpm install -g @new-scope/kin",
				},
			],
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		tempDir = temp;
		process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
		process.env.KIN_PACKAGE_DIR = storePackageDir;
	...
	test("self-updates renamed yarn global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "yarn",
			args: ["global", "add", "@new-scope/kin"],
			display: "yarn global remove @mariozechner/kin-coding-agent && yarn global add @new-scope/kin",
			steps: [
				{
					command: "yarn",
					args: ["global", "remove", "@mariozechner/kin-coding-agent"],
					display: "yarn global remove @mariozechner/kin-coding-agent",
				},
				{
					command: "yarn",
					args: ["global", "add", "@new-scope/kin"],
					display: "yarn global add @new-scope/kin",
				},
			],
		});
	});
	...
	test("self-updates renamed bun global installs by removing the old package first", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "@new-scope/kin"],
			display: "bun uninstall -g @mariozechner/kin-coding-agent && bun install -g @new-scope/kin",
			steps: [
				{
					command: "bun",
					args: ["uninstall", "-g", "@mariozechner/kin-coding-agent"],
					display: "bun uninstall -g @mariozechner/kin-coding-agent",
				},
				{
					command: "bun",
					args: ["install", "-g", "@new-scope/kin"],
					display: "bun install -g @new-scope/kin",
				},
			],
		});
	});
