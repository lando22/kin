...
Use `/logout` to clear credentials. Tokens are stored in `~/.kin/agent/auth.json` and auto-refresh when expired.
...
Reference for environment variables and `auth.json` keys: [`const envMap`](https://github.com/lando22/kin/blob/main/packages/ai/src/env-api-keys.ts) in [`packages/ai/src/env-api-keys.ts`](https://github.com/lando22/kin/blob/main/packages/ai/src/env-api-keys.ts).

#### Auth File

Store credentials in `~/.kin/agent/auth.json`:
...
```bash
kin --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
kin --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
kin --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
kin --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```
