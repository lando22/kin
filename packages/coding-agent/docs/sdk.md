...
```bash
npm install @landongarrison/kin-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@landongarrison/kin-coding-agent";
...
The `Agent` class (from `@landongarrison/kin-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.kin/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.kin/extensions/`)
- Project skills:
  - `.kin/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.kin/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.kin/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@landongarrison/kin-coding-agent";
...
// Default: uses ~/.kin/agent/auth.json and ~/.kin/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@landongarrison/kin-coding-agent";
...
- Built-in tool names: `read`, `bash`, `edit`, `write`, `definition`
...
import { createAgentSession, defineTool } from "@landongarrison/kin-coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.kin/agent/extensions/`, `.kin/extensions/`, and settings.json extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@landongarrison/kin-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@landongarrison/kin-coding-agent";
...
1. Global: `~/.kin/agent/settings.json`
2. Project: `<cwd>/.kin/settings.json`
