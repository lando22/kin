...
> **Placement for /reload:** Put extensions in `~/.kin/agent/extensions/` (global) or `.kin/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.kin/agent/extensions/my-extension.ts`:
...
| Location | Scope |
|----------|-------|
| `~/.kin/agent/extensions/*.ts` | Global (all projects) |
| `~/.kin/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.kin/extensions/*.ts` | Project-local |
| `.kin/extensions/*/index.ts` | Project-local (subdirectory) |
...
| Package | Purpose |
|---------|---------|
| `@landongarrison/kin-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@landongarrison/kin-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@landongarrison/kin-tui` | TUI components for custom rendering |
...
This pattern makes the fetched models available during normal startup and to `kin --list-models`.
...
The `systemPromptOptions` field gives extensions access to the same structured data Pi uses to build the system prompt. This lets you inspect what Kin has loaded — custom prompts, guidelines, tool snippets, context files, skills — without re-discovering resources or re-parsing flags. Use it when your extension needs to make deep, informed changes to the system prompt while respecting user-provided configuration.
...
Before `tool_call` runs, kin waits for previously emitted Agent events to finish draining through `AgentSession`. This means `ctx.sessionManager` is up to date through the current assistant tool-calling message.
...
```typescript
import { isToolCallEventType } from "@landongarrison/kin-coding-agent";
...
```typescript
import { isBashToolResult } from "@landongarrison/kin-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@landongarrison/kin-coding-agent";
...
```typescript
import { SessionManager } from "@landongarrison/kin-coding-agent";
...
If multiple extensions register the same command name, kin keeps them all and assigns numeric invocation suffixes in load order, for example `/review:1` and `/review:2`.
...
```typescript
import type { AutocompleteItem } from "@landongarrison/kin-tui";
...
If you need to discover models from a remote endpoint, prefer an async extension factory over deferring the fetch to `session_start`. kin waits for the factory before startup continues, so the registered models are available immediately, including to `kin --list-models`.
...
```typescript
import { withFileMutationQueue } from "@landongarrison/kin-coding-agent";
...
**Important:** Use `StringEnum` from `@landongarrison/kin-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
Extensions can override built-in tools (`read`, `bash`, `edit`, `write`, `definition`) by registering a tool with the same name. Interactive mode displays a warning when this happens.
...
kin --no-builtin-tools -e ./my-extension.ts
```
...
- [read.ts](https://github.com/lando22/kin/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/lando22/kin/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/lando22/kin/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/lando22/kin/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [definition.ts](https://github.com/lando22/kin/blob/main/packages/coding-agent/src/core/tools/definition.ts) - `DefinitionToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@landongarrison/kin-coding-agent";
...
```typescript
import { createBashTool } from "@landongarrison/kin-coding-agent";
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/lando22/kin/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@landongarrison/kin-coding-agent";
...
```typescript
import { Text, Component } from "@landongarrison/kin-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@landongarrison/kin-coding-agent";
import { matchesKey } from "@landongarrison/kin-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@landongarrison/kin-coding-agent";
