# @earendil-works/kin-agent-core

Stateful agent with tool execution and event streaming. Built on `@earendil-works/kin-ai`.

## Installation

```bash
npm install @earendil-works/kin-agent-core
```
...
```typescript
declare module "@earendil-works/kin-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@earendil-works/kin-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@earendil-works/kin-agent-core";
