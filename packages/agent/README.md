# @landongarrison/kin-agent-core

Stateful agent with tool execution and event streaming. Built on `@landongarrison/kin-ai`.

## Installation

```bash
npm install @landongarrison/kin-agent-core
```
...
```typescript
declare module "@landongarrison/kin-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@landongarrison/kin-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@landongarrison/kin-agent-core";
